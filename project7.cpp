...
main( int argc, char *argv[ ] )
{
	...
	if( me == BOSS )
	{
		for( int dst = 0; dst < NumCpus; dst++ )
		{
			...
			MPI_Send( &BigSignal[dst*PPSize], PPSize, MPI_FLOAT, dst, 1337, MPI_COMM_WORLD );
		}
	}
	else
	{
		MPI_Recv( PPSignal, PPSize, MPI_FLOAT, BOSS, 1337, MPI_COMM_WORLD, &status );
	}
...
DoOneLocalAutocorrelation( int me )
{
	...
	if( DEBUG )	fprintf( stderr, "Node %3d entered DoOneLocalAutocorrelation( ) with %d\n", me, PPSize );
